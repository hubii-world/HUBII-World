...
# Load dataset
file_path = "C://Biosignal_Analysis//data//HubiiRec_HR_Testdata_Large.csv"
...
# Define window length and step size
...
for window in windows:
    ...
    if len(rr_intervals) > 1:  # Ensure at least two RR intervals for HRV calculation
        try:
            r_peaks = nk.intervals_to_peaks(rr_intervals, sampling_rate=1000)
            hrv_features = nk.hrv_time(r_peaks, sampling_rate=1000)  # Sampling rate = 1000Hz (RR is in ms)
            hrv_results.append(hrv_features)
            timestamps.append(window["SystemTime"].max())  # Store max timestamp of window
...
if hrv_results:
    ...
    hrv_df["MaxTimestamp"] = timestamps

    # Save the results to a CSV file
    hrv_results_file = "C://Biosignal_Analysis//data//HRV_Features.csv"
    hrv_df.to_csv(hrv_results_file, sep=';', index=False)
